...
- **Install** — Use `pnpm install --frozen-lockfile --ignore-scripts` (or `pnpm run deps`). `pnpm-workspace.yaml` sets `frozenLockfile: true` and `ignoreScripts: true`. To refresh the lockfile after dependency changes, use `pnpm install --no-frozen-lockfile --ignore-scripts`. After a fresh clone, run `pnpm exec husky install` once if you need git hooks. Keep `overrides` in `pnpm-workspace.yaml` aligned with advisories (`pnpm audit`).
