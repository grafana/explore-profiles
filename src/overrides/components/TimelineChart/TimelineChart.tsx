...
class TimelineChart extends ReactFlot<TimelineChartProps> {
  ...
  componentDidUpdate() {
    // Originally this was done in `componentDidMount`
    // However it seems the dom node may not be available
    $(`#${this.props.id}`).bind('plotselected', (event: unknown, ranges: { xaxis: { from: number; to: number } }) => {
      this.props.onSelect(
  ...
  componentWillUnmount() {
    $(`#${this.props.id}`).unbind('plotselected');
  }
  ...
  render() {
    ...
    return (
      <div data-testid={this.props['data-testid']} className={this.props.className} id={this.props.id} style={style} />
    );
  }
}
