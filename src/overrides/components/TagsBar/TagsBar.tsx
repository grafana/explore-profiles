...
import Dropdown, { SubMenu, MenuItem, FocusableItem, MenuGroup } from '@pyroscope/ui/Dropdown';
...
interface TagsBarProps extends TagsDropdownProps {
  /** callback for when the same query is submitted again */
  onRefresh: () => void;
}

function EmptyTagsDropdown({ text }: { text: string }) {
  return (
...
function TagsDropdown({ query, tags, onSelectedLabel, onSetQuery }: TagsDropdownProps) {
  switch (tags?.type) {
    case 'loading': {
      return <EmptyTagsDropdown text="Loading" />;
    }

    case 'loaded': {
      if (!Object.keys(tags?.tags || {}).length) {
        return <EmptyTagsDropdown text="No tags available" />;
      }
...
function LabelsSubmenu({ query, labels: tags, onSelectedLabel, onSelectedLabelValue }: LabelsSubmenuProps) {
      ...
      case 'loaded': {
        ...
        return (
          values
            ...
            .filter((n) => {
              const f = filter[labelName] ? filter[labelName].trim().toLowerCase() : '';
            ...
            .map((labelValue) => {
              return (
                <MenuItem
                  ...
                  className={isLabelInQuery(query, labelName, labelValue) ? 'active' : ''}
                >
                  {labelValue}
                </MenuItem>
              );
  ...
  const Items = Object.entries(tags).map(([tag, tagValues]) => {
    ...
    return (
      <SubMenu
        ...
        label={() => (
          <span className="tag-content" aria-hidden onClick={() => onSelectedLabel(tag)}>
            {tag}
          </span>
        )}
      >
