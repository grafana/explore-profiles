...
import { AppNameContext, PageTitle } from '../PageTitle';
...
describe('PageTitle', () => {
  describe("there's no app name in context", () => {
    it('defaults to Pyroscope', async () => {
      ...
      await waitFor(() => expect(document.title).toEqual('mypage | Pyroscope'), waitForOpts);
    });
  });
  ...
  describe("there's an app name in context", () => {
    it('suffixes the title with it', async () => {
      ...
      await waitFor(() => expect(document.title).toEqual('mypage | myapp'), waitForOpts);
    });
  });
});
