...
import { changeTimeZoneOffset, selectTimezoneOffset } from '@pyroscope/redux/reducers/ui';
...
export function useTimeZone() {
  ...
  const browserTimeLabel = useMemo(() => {
    ...
    return `Browser Time (UTC${offset < 0 ? '+' : '-'}${
      ('00' + Math.floor(absOffset / 60)).slice(-2) + ':' + ('00' + (absOffset % 60)).slice(-2)
    })`;
  }, [offset]);
  ...
  return {
    ...
    changeTimeZoneOffset: (value: number) => dispatch(changeTimeZoneOffset(value)),
  };
}
